...
const allowListByChain: Record<number, Set<string>> = {
  [infra.optimism.id]: new Set([
...
const allowList = allowListByChain[chain.id] ?? new Set<string>();
...
export async function getAllowTokens() {
  ...
  if (chain.id !== infra.optimism.id) return allowTokens;
  try {
