...
export default function useSimulateProposal({
  ...
  const proposalsSlot =
    account === undefined || nonce === undefined
      ? undefined
      : hexToBigInt(
          keccak256(
            encodeAbiParameters(
  ...
  const proposalDataSlot =
    proposalsSlot === undefined
      ? undefined
      : hexToBigInt(keccak256(encodeAbiParameters([{ type: "uint256" }], [proposalsSlot + 4n])));
  const proposalDataBytes = proposalData && hexToBytes(proposalData);
  const executeProposal = useSimulateContract({
    ...
    stateOverride: [
      {
        ...
        state:
          ...
          nonce === undefined ||
          proposalsSlot === undefined ||
          proposalDataSlot === undefined ||
          proposalDataBytes === undefined
            ? undefined
            : [
                {
                  // nonces[account]
                  slot: keccak256(encodeAbiParameters([{ type: "string" }, { type: "uint256" }], [account, 3n])),
                  value: encodeAbiParameters([{ type: "uint256" }], [nonce]),
                },
                {
                  // queueNonces[account]
                  slot: keccak256(encodeAbiParameters([{ type: "string" }, { type: "uint256" }], [account, 4n])),
    ...
    query: { enabled: enabled && proposalsSlot !== undefined },
  });
