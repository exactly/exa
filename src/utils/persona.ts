...
export async function createInquiry(credential: Credential, navigator: AppNavigationProperties) {
  if (Platform.OS === "web") {
  ...
  Inquiry.fromTemplate(KYC_TEMPLATE_ID)
    ...
    .onCanceled(() => {
      ...
      navigator.replace("(home)", { screen: "index" });
    })
    .onComplete(() => {
      queryClient.invalidateQueries({ queryKey: ["kyc", "status"] }).catch(reportError);
...
export async function resumeInquiry(inquiryId: string, sessionToken: string, navigator: AppNavigationProperties) {
  if (Platform.OS === "web") {
    const url = await getKYCLink(KYC_TEMPLATE_ID);
    if (await sdk.isInMiniApp()) {
      await sdk.actions.openUrl(url);
