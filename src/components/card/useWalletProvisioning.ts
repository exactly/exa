...
export default function useWalletProvisioning(lastFour: string, displayName: string) {
  ...
  async function addToAppleWallet() {
    await provision(async (cardData) => {
  ...
  async function addToGoogleWallet() {
    await provision((cardData) => MeaPushProvisioning.GooglePay.pushCard(cardData, displayName, {}));
  }

  return { eligible, provisioning, isPending, addToAppleWallet, addToGoogleWallet };
}
