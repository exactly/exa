...
import { divWad, fixedRepayAssets, fixedRepayPosition, min, mulWad, WAD, withdrawLimit } from "@exactly/lib";
...
export default function Pay() {
  ...
  const { data: routeFrom } = useQuery({
    queryKey: ["lifi", "routeFrom", mode, account, selectedAsset.address, repayMarket?.asset, repayMarketAvailable],
    queryFn: () => {
      if (!repayMarketAvailable) throw new Error("no repay market available");
      switch (mode) {
        case "crossRepay":
  ...
  const maxPositionAssets = useMemo(() => {
    const availableUSDC = repayMarket?.market === exaUSDC?.market ? repayMarketAvailable : routeFrom?.toAmount;
    return borrow && balancerUSDCBalance && fixedRepaySnapshot && maturity && repayMarket && availableUSDC
      ? min(
          fixedRepayPosition(
            fixedRepaySnapshot,
            Number(maturity),
  ...
  const handlePayment = useCallback(() => {
    ...
    setDisplayValues({
      amount: Number(withUSDC ? 0n : route?.fromAmount) / 10 ** repayMarket.decimals,
        ...
        <View fullScreen gap="$s5" paddingTop="$s4_5">
          ...
          >
            <YStack gap="$s4" paddingBottom={insets.bottom}>
              ...
              >
                <Button.Text>
                  {simulationError || positionAssets > maxPositionAssets
                    ? "Cannot proceed"
                    : loading
                      ? "Please wait..."
                      : "Confirm payment"}
