...
import { proposalManagerAbi, useReadProposalManagerDelay } from "@exactly/common/generated/hooks";
...
export default function Confirm() {
  ...
  const {
    request: bridgePropose,
    error: bridgeProposeError,
    refetch: refetchBridgePropose,
  } = useSimulateProposal({
    account: address,
    amount: fromAmount,
    market: market?.market,
    ...
    minAmountOut: 0n,
    route: route?.data,
    enabled: enableSimulations && routed && !!market && !!address && !!route && !!neutralAsset && fromAmount > 0n,
  });
  ...
  const {
    ...
    enabled: enableSimulations && !!calls,
  ...
  const receivedAmount = receivedTokens.toLocaleString(language, { maximumFractionDigits: 8 });
  const receivedSymbol = received?.symbol ?? "";
  const receivedSize = room && longest ? Math.min(36, Math.max(14, Math.floor((probe * room * 0.9) / longest))) : 36;
  ...
  const { data: proposalDelay } = useReadProposalManagerDelay({ query: { enabled: !!market } });
  const arrivalMinutes = routed
    ? Math.max(1, Math.ceil((route?.estimate.executionDuration ?? 60) / 60))
    : Math.max(1, Math.ceil(Number(proposalDelay ?? 0n) / 60));
  ...
  const outage = !!prepareError && classifyError(prepareError).network !== undefined;
  useEffect(() => {
  ...
  const fee =
    insufficientGas && nativeToken
      ? routeValue <= (nativeBalance ?? 0n) && gasToken && feeIsSource && paymasterAddress && erc20GasReserve > 0n
      ...
      <SafeView fullScreen>
        <View gap="$s4_5" fullScreen padded>
          ...
          <YStack flex={1} position="relative" gap="$s4">
            ...
            >
              <YStack gap="$s4">
                <YStack gap="$s4" alignItems="center" paddingVertical="$s4_5">
                  {received ? (
                    <>
                      <YStack
                        aria-hidden
                        ...
                        opacity={0}
                        onLayout={({ nativeEvent }) => {
                          setLongest(nativeEvent.layout.width);
                        }}
                      >
                        <Text fontSize={probe} numberOfLines={1}>
                          {receivedAmount}
                        </Text>
                        <Text fontSize={probe} numberOfLines={1}>
                          {receivedSymbol}
                      ...
                      <AssetLogo uri={received.logoURI} symbol={received.symbol} width={40} height={40} />
                      <Text
                        primary
                        width="100%"
                        textAlign="center"
                        numberOfLines={2}
                        fontSize={receivedSize}
                        lineHeight={Math.round(receivedSize * 1.3)}
                      >
                ...
                <YStack gap="$s4">
                  ...
                  <YStack gap="$s3_5">
                    <XStack gap="$s3_5" alignItems="center" justifyContent="space-between">
                      <Text footnote secondary flexShrink={0}>
                        {t("To")}
                      </Text>
                      <YStack
                        flexShrink={1}
                        maxWidth={expanded ? "50%" : undefined}
                        alignItems="flex-end"
                        cursor="pointer"
                      ...
                      >
                        <XStack gap="$s3" alignItems="center">
                          <View borderRadius="$r_0" overflow="hidden">
                            <Blocky seed={receiver} scale={2} />
                          </View>
                          <Text subHeadline primary mono={!ens} flexShrink={1} textAlign="right">
                            {ens || (expanded ? receiver : shortenHex(receiver, 8, 10))}
                          </Text>
                        </XStack>
                        {!!ens && (
                          <Text caption2 secondary mono textAlign="right">
                            {expanded ? receiver : shortenHex(receiver, 8, 10)}
                          </Text>
                        )}
                      </YStack>
                    </XStack>
                    <XStack gap="$s3_5" alignItems="center">
                      ...
                      <XStack gap="$s3" alignItems="center">
                        <ChainLogo chainId={destinationChain} size={20} />
                    ...
                    {routed && (
                      <>
                        ...
                        <Detail label={t("Minimum received")}>
                    ...
                    <Detail label={t("Estimated arrival")}>
                      <Text subHeadline primary>
                    ...
                    <Text body secondary textAlign="center">
                      {failure === "route"
                        ...
                        : failure === "liquidity"
                          ...
                          : failure === "queued"
                            ? t("Wait for it to be processed before sending again")
                  ...
                  <Button
                    ...
                    onPress={() => {
                      switch (failure) {
                        ...
                        case "queued":
                          if (market) {
                            (routed ? refetchBridgePropose() : refetchPropose()).catch(reportError);
                          } else {
                            (isNativeTransfer || routed
                          ...
                          break;
                        default:
                          (destinationFailure ? refetchTokens() : refetchRoute()).catch(reportError);
                      }
                    }}
                  >
                    <Button.Text>
...
function Detail({ label, children }: { children: React.ReactNode; label: string }) {
  return (
    <XStack gap="$s2" alignItems="center">
