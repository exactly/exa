...
export default function GettingStarted({ hasFunds, hasKYC }: { hasFunds: boolean; hasKYC: boolean }) {
    ...
    mutationFn: async () => {
      ...
      try {
        ...
        if (typeof result !== "string") {
          resumeInquiry(result.inquiryId, result.sessionToken, appNavigator).catch(reportError);
        }
      } catch (error) {
        ...
        if (error.text === "kyc required" || error.text === "kyc not found" || error.text === "kyc not started") {
          await createInquiry(credential, appNavigator);
          return;
        }
        reportError(error);
      }
  ...
  function handleStepPress() {
    ...
    switch (currentStep?.id) {
      case "add-funds":
        appNavigator.navigate("add-funds", { screen: "add-crypto" });
        break;
          ...
          <XStack gap={2} alignItems="center">
            <Pressable
              ...
              onPress={() => {
                ...
                appNavigator.navigate("getting-started");
              }}
            >
