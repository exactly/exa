...
export default function AssetSelector({
  ...
  const { assets, externalAssets, markets, isPending, isExternalPending } = usePortfolio({ sortBy });

  if (assets.length === 0) {
    if (isPending || isExternalPending || !markets) {
      return (
  ...
  return (
    <YStack gap="$s2" borderWidth={1} borderRadius="$r3" borderColor="$borderNeutralSeparator">
      ...
      >
        ...
        {isExternalPending ? <AssetSkeleton /> : null}
      </ToggleGroup>
    </YStack>
  );
}
