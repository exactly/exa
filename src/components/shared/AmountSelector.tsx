...
export default function AmountSelector({ onChange }: { onChange: (value: bigint) => void }) {
  const usdInputReference = useRef<HTMLInputElement | null>(null); // eslint-disable-line @eslint-react/naming-convention/ref-name
        ...
        <Field name="assetInput" validators={{ onChange: pipe(string(), nonEmpty("empty amount")) }}>
          {({ state: { value } }) => (
            <Input
              ...
              onChange={({ target }) => handleAssetChange((target as { value?: string }).value ?? "")}
              value={value}
          ...
          {({ state: { value } }) => (
            ...
            >
              <Input
                ...
                inputMode="decimal"
                onChange={({ target }) => handleUsdChange((target as { value?: string }).value ?? "")}
                placeholder="USD"
                value={value}
                onBlur={() => {
                  setOverlayShown(true);
