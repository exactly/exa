...
export default function Blockie({
  ...
  const resolvedSize = normalizeInteger(size, defaultSize);
  const resolvedScale = normalizeInteger(scale, defaultScale);
...
function renderIcon(options: {
  ...
  for (const [index, item] of imageData.entries()) {
    ...
    pixels.push(
      <Rect
        key={`cell-${index}`}
        x={column * scale} // eslint-disable-line @typescript-eslint/no-deprecated
        y={row * scale} // eslint-disable-line @typescript-eslint/no-deprecated
        width={scale}
        height={scale}
        fill={fill}
      />,
...
function buildOptions(options: {
  ...
  const seed = options.seed ?? Math.floor(Math.random() * 10 ** 16).toString(16);
  const rand = createRng(seed);
  const size = normalizeInteger(options.size, defaultSize);
  const scale = normalizeInteger(options.scale, defaultScale);
