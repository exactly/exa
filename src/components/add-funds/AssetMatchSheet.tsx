...
import { ArrowRight, Check, CheckCircle } from "@tamagui/lucide-icons-2";
...
export default function AssetMatchSheet({
  ...
  const { data: sourceNetwork } = useQuery({
    ...lifiChainsOptions,
    select: (chains) => chains.find((c) => c.id === (sourceChainId ?? chain.id))?.name,
  });
  return (
          ...
          <YStack gap="$s5">
            ...
            <Text subHeadline secondary>
              {t(
                ...
                {
                  source: sourceSymbol,
                  network: sourceNetwork ?? chain.name,
            ...
            <XStack justifyContent="center" alignItems="center" gap="$s5" paddingVertical="$s4">
              <View position="relative">
                <AssetLogo symbol={sourceSymbol} chainId={sourceChainId} width={48} height={48} />
