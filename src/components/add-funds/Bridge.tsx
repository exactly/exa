...
export default function Bridge() {
  ...
  const senderConfig = useLocalSearchParams().sender === "external" ? externalConfig : ownerConfig;
  ...
  const {
    ...
    queryFn: () => {
      if (!senderAddress || !account || !selectedSource || !sourceToken || !destinationToken || sourceAmount === 0n)
  ...
  const {
    ...
    mutationFn: async (from: RouteFrom) => {
      if (!senderAddress || !selectedSource) throw new Error("missing bridge context");
      ...
      try {
        ...
        if (approval) await sendTransactionAsync({ to: getAddress(selectedSource.address), data: approval });
        await sendTransactionAsync({ to: from.to, data: from.data, value: from.value });
        setBridgeStatus("Bridge transaction submitted");
      }
    },
    onSuccess: async () => {
