...
import { Check, ChevronDown, Network, Search } from "@tamagui/lucide-icons-2";
...
export default function AssetSelectSheet({
  ...
  const filteredGroups = useMemo(() => {
    ...
    const activeChain = groups.some((group) => group.chain.id === chainFilter) ? chainFilter : undefined;
    return groups
      .filter((group) => activeChain === undefined || group.chain.id === activeChain)
      .map((group) => {
        ...
        <View padded paddingTop="$s4" paddingBottom={0} flex={1} gap="$s4_5">
          ...
          <View position="relative" zIndex={10}>
            ...
            >
              ...
              {groups.length > 1 && (
                <Pressable role="button" aria-label={t("Filter by network")} onPress={() => setFilterOpen(!filterOpen)}>
...
function FilterRow({
  ...
  return (
    <Pressable role="button" aria-label={label} onPress={onPress}>
      <XStack
        padding="$s3_5"
        alignItems="center"
        gap="$s3_5"
