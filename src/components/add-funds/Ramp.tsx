...
import { ArrowLeft, Banknote, Clock, Copy, Percent, QrCode, Repeat } from "@tamagui/lucide-icons-2";
...
export default function Ramp() {
  ...
  const bridgeFees =
    typedProvider === "bridge"
      ? deposits
          .filter((d) => d.displayName in rampFees.bridge)
          .map((d) => `${d.displayName}: ${rampFees.bridge[d.displayName as keyof typeof rampFees.bridge].fee}`)
          .join(". ")
      : undefined;
