...
deployment service account, runtime service accounts, signer kms keys, secret containers, minimum iam grants scoped to
matching github environments, and default-service-account organization policy, and creates no application runtime
resource or secret value. a one-time human-authenticated local update creates these resources
...
run developer, and service usage admin project-wide; grants service account user on each runtime account; and conditions
custom secret permissions to the stack prefix. meta owns each signer key ring and key, while github automation may only
get and set iam policy on keys in its stack's signer ring so the runtime stack can grant each signer worker the
signer/verifier role. github automation receives no key creation or rotation permission. meta also prevents automatic
editor grants to default service accounts. the one-time migration uses explicit pulumi export, stack initialization, and import commands;
