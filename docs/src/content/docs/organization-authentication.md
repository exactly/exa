...
## How to send an invitation to the Integrator organization
...
## How to accept an invitation from the integrator organization

Use the invite id generated by the owner or the admin role of the organization and your private key.
Pass the invited email in the SIWE verify body so the session is bound to it; otherwise the invitation cannot be accepted.
    ...
    await authClient.siwe.verify(
      {
        ...
        walletAddress: owner.address,
        chainId,
        email: "bob@integrator.com", // must match the invited email
      },
      {
        onSuccess: async (context) => {
          const headers = new Headers();
