...
function encrypt(payload: string) {
  ...
  const cipher = crypto.createCipheriv("aes-256-gcm", aesKey, iv); // cspell:disable-line
...
-----END PUBLIC KEY-----`;
  ...
  const key = crypto.publicEncrypt(
    {
      key: publicKey,
      padding: crypto.constants.RSA_PKCS1_OAEP_PADDING, // cspell:disable-line
      oaepHash: "sha256", // cspell:disable-line
    },
    aesKey,
  );
...
authClient.siwe
  ...
  .then(async ({ data: nonceResult }) => {
    ...
    const statement = `I apply for KYC approval on behalf of address ${exaAccountUserAddress.toLocaleLowerCase()}`;
    const nonce = nonceResult?.nonce ?? "";
    const message = createSiweMessage({
      statement,
      resources: ["https://exactly.github.io/exa"],
    ...
    console.log("application payload", { ...encryptedPayload, verify });
  })
  .catch((error: unknown) => {
    console.error("nonce error", error);
  });
