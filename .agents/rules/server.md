...
- **migrations**: all schema changes must be accompanied by a migration file generated by `drizzle-kit`. run `pnpm --filter server db:push`. never edit the database schema manually.
...
- **api generation**: run `pnpm --filter server openapi` to update the openapi spec.
