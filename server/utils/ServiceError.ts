export default class ServiceError extends Error {
  constructor(
    ...
    if (type === undefined && detail === undefined && typeof cause === "string" && cause) {
      try {
        ...
        if (typeof payload === "object" && payload !== null && !Array.isArray(payload)) {
          ...
          if (typeof p.error === "string" && p.error !== "Error") type = p.error;
          else if (typeof p.code === "string" && p.code.endsWith("Error")) type = p.code;
    ...
    super(detail ?? `${status}`, { cause });
    this.name = type ? `${service}${type.replace(/Error$/, "")}` : `${service}${status}`;
  }
}
