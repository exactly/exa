...
export async function redactDocuments(referenceId: string) {
  const account = await getUnknownAccount(referenceId);
  if (!account.data[0]) throw new Error("account not found");
  return request(
    object({ data: object({ id: string() }) }),
    `/accounts/${account.data[0].id}`,
    {
      data: {
...
function evaluateAccount(
  ...
  switch (scope) {
    ...
    case "manteca": {
      ...
      if (!userIdClasses?.length) throw new Error(scopeValidationErrors.ACCOUNT_NOT_CREATED);
...
export function isMissingOrNull<TInput>(issue: BaseIssue<TInput>): boolean {
  return (
    ...
    (issue.kind === "validation" && issue.type === "min_length" && issue.received === "0")
  );
}
