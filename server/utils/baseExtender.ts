...
import type { Hash } from "@exactly/common/validation";

export default function baseExtender<TAccount extends LocalAccount>(
  client: WalletClient<HttpTransport, typeof chain, TAccount> & {
    account: TAccount;
  },
) {
  return {
        ...
        startSpan({ forceTransaction: true, ...spanOptions }, async (span) => {
          try {
            ...
            const serializedTransaction = await startSpan({ name: "sign transaction", op: "tx.sign" }, () =>
              // eslint-disable-next-line @typescript-eslint/no-explicit-any -- viem generic type inference limitation
              client.signTransaction(prepared as any),
            );
