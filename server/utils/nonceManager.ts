...
export function createNonceManager({ source }: CreateNonceManagerParameters): NonceManager & {
  ...
  return {
    async consume({ address, chainId, client }) {
      return startSpan({ name: "consume nonce", op: "nonce.manager.consume" }, async (span) => {
    ...
    increment({ address, chainId }) {
      startSpan({ name: "increment nonce", op: "nonce.manager.increment" }, (span) => {
        const key = getKey({ address, chainId });
        span.setAttribute("exa.key", key);
    ...
    async get({ address, chainId, client }) {
      return startSpan({ name: "get nonce", op: "nonce.manager.get" }, async (span) => {
        const key = getKey({ address, chainId });
        span.setAttribute("exa.key", key);
    ...
    reset({ address, chainId }) {
      startSpan({ name: "reset nonce", op: "nonce.manager.reset" }, (span) => {
        const key = getKey({ address, chainId });
        span.setAttribute("exa.key", key);
        span.setAttribute("exa.delta", deltaMap.get(key));
    ...
    hardReset({ address, chainId }) {
      startSpan({ name: "hard reset", op: "nonce.manager.hard" }, (span) => {
        const key = getKey({ address, chainId });
        span.setAttribute("exa.key", key);
        span.setAttribute("exa.delta", deltaMap.get(key));
        span.setAttribute("exa.promise", !!promiseMap.get(key));
