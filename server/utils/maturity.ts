...
import { addBreadcrumb, captureException, captureMessage, startSpan } from "@sentry/node";
...
const sendMaturityRemindersSchema = object({ accounts: array(Address), maturity: number(), window: windowSchema });
...
const worker = observe(
  new Worker<CheckDebts | ScanChunk>(
    ...
    (job) =>
      startSpan(
        { name: "maturity.worker", op: "queue.process", attributes: { job: job.name, window: job.data.window } },
        async (span) => {
          switch (job.name) {
            case "check-debts": {
            ...
            case "scan-chunk": {
              ...
              if (accounts.length > 0)
                await notificationQueue.add(
                  ...
                  { accounts, maturity: scan.maturity, window: scan.window },
