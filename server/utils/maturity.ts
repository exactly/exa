...
export async function processor(job: Job<CheckDebts>) {
  return startSpan(
      ...
      switch (job.name) {
        case "check-debts":
          try {
            ...
            const threshold = window === "24h" ? 24 * 3600 : 3600;
            if (maturity - now < 0 || maturity - now > threshold) {
              addBreadcrumb({
            ...
            for (let offset = 0; ; offset += CHUNK_SIZE) {
              ...
              const notifications = results.accounts
                .filter(({ hasDebt }) => hasDebt)
                .map(async ({ account }) => {
                  const key = `notification:sent:${account}:${maturity}:${window}`;
...
async function checkDebts(chunk: { account: string }[], maturity: number) {
  ...
  const accounts = chunk.map(({ account }, index) => ({
    account,
    hasDebt:
