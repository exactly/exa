...
async function businessApplication(
  ...
  if (!clientIp.success) throw new BusinessApplicationError("missing valid client IP address", "bad request");
  const profile = await businessProfile(credentialId, persona);
  const fields = requireFields(BusinessApplicationFields, profile.fields);
  const person = {
  ...
  return requireFields(CreateCompanyApplicationRequest, {
