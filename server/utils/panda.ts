...
import { BusinessApplicationError, isBlank, requireFields } from "./persona";
...
function companyAddress(fields: InferOutput<typeof BusinessFields>) {
  return {
    line1: fields.street_1_2,
    line2: fields.street_2,
    city: fields.city_2,
    region: fields.subdivision_2,
    postalCode: fields.postal_code_2,
    countryCode: fields.country_code_2,
  };
}

function ubos(fields: Record<string, unknown>) {
  const indexes = [
    ...new Set(
      Object.keys(fields)
        .filter((key) => key.startsWith("ubo_"))
        .map((key) => Number(/_(\d+)$/.exec(key)?.[1] ?? 1)),
    ),
  ].toSorted((a, b) => a - b);
  return indexes.flatMap((index) => {
    const address = index + 2;
    const block = {
      birth_date: fields[index === 1 ? "ubo_birth_date" : `ubo_birth_date_${index}`],
      city: fields[`city_${address}`],
      country_code: fields[`country_code_${address}`],
      country_id: fields[`ubo_country_id_${index}`],
      email: fields[`ubo_email_${index}`],
      id: fields[`ubo_id_${index}`],
      last_name: fields[`ubo_last_name_${index}`],
      name: fields[`ubo_name_${index}`],
      postal_code: fields[`postal_code_${address}`],
      street_1: fields[`street_1_${address}`],
      street_2: fields[`street_2_${index + 1}`],
      subdivision: fields[`subdivision_${address}`],
    };
    if (Object.values(block).every(isBlank)) return [];
    const ubo = requireFields(UboFields, block);
    return [
      {
        address: {
          line1: ubo.street_1,
          line2: ubo.street_2,
          city: ubo.city,
          region: ubo.subdivision,
          postalCode: ubo.postal_code,
          countryCode: ubo.country_code,
        },
        birthDate: ubo.birth_date,
        countryOfIssue: ubo.country_id,
        email: ubo.email,
        firstName: ubo.name,
...
function businessApplication(
  ...
  const result = safeParse(CreateCompanyApplicationRequest, {
    ...
    ultimateBeneficialOwners: ubos(profile.fields), // cspell:ignore ubos
    sourceKey: "EXA",
...
const UboFields = object({
  birth_date: requiredValue,
  city: requiredValue,
  country_code: requiredValue,
  country_id: requiredValue,
  email: requiredValue,
  id: requiredValue,
  last_name: requiredValue,
  name: requiredValue,
