...
export async function getQuote(coinPair: string): Promise<v.InferOutput<typeof shared.QuoteResponse>> {
...
): v.InferOutput<typeof shared.DepositDetails>[] {
...
export async function getProvider(
  ...
  pendingTasks: v.InferOutput<typeof shared.PendingTask>[];
}> {
      ...
      if (error instanceof Error && Object.values(ErrorCodes).includes(error.message)) {
        switch (error.message) {
          ...
          case ErrorCodes.BAD_KYC_ADDITIONAL_DATA: {
            const inquiryTask: v.InferOutput<typeof shared.PendingTask> = {
              type: "INQUIRY",
...
function getDocumentId(inquiry: v.InferOutput<typeof Inquiry>) {
