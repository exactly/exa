...
export default function alchemy(key: string) {
  ...
  return { findWebhook, createWebhook, addWebhookAddresses, updateWebhookAddresses, headers };
  ...
  async function addWebhookAddresses(id: string | undefined, addresses: Address[]) {
    ...
    return updateWebhookAddresses(id, addresses);
  }

  async function updateWebhookAddresses(id: string | undefined, add: Address[], remove: Address[] = []) {
    if (!id) return;
