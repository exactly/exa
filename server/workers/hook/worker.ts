  ...
  return createWorker<Job>({
    ...
    async process(job, span) {
      ...
      const data = v.safeParse(
        v.object({ receipt: v.optional(v.object({ blockNumber: v.number(), transactionHash: v.string() })) }),
        job.data,
      );
      if (!data.success) {
      ...
      const config = v.safeParse(webhookConfig, user.source.config);
      if (!config.success) {
      ...
      const failures = await Promise.all(
        Object.values(config.output.webhooks).map(async (target) => {
          try {
            const response = await fetch(
              ...
              })() ?? target.url,
              {
                ...
                headers: {
                  ...
                  Signature: createHmac("sha256", target.secret).update(JSON.stringify(outbound.output)).digest("hex"),
                },
      ...
      if (failures.length > 0) throw new AggregateError(failures, "webhook failed");
    },
  });
}
