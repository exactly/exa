...
export default async function backfillPax(dryRun: boolean, delayMs = 250): Promise<BackfillResult[]> {
  ...
  for (const credential of usersWithCards) {
    ...
    try {
      ...
      if (!inquiry) {
        ...
        if (dryRun) {
          // eslint-disable-next-line no-console -- cli script
          console.log(`[DRY-RUN] ${account}: Would add to Pax with data:`, {
            ...capitaData,
    ...
    if (delayMs > 0 && usersWithCards.indexOf(credential) < usersWithCards.length - 1) {
...
const maskPII = (value: string | null | undefined): string => {
  if (!value) return "";
