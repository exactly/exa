...
export default new Hono().get(
  ...
  async (c) => {
    ...
    const marketAsset = (address: Hex) =>
      [...markets.values()].find(({ asset }) => asset.toLowerCase() === address.toLowerCase())?.asset;
    ...
    const tokens = await lifiTokens(chain.id, [
      ...new Set(swaps.flatMap(({ fromAssetId, toAssetId }) => [fromAssetId, toAssetId])),
    ]).catch((error: unknown) => {
    ...
    return c.json(
      [
        ...
        ...swaps.map(({ blockNumber, ...event }) => {
          ...
          const { toAssetId, fromAssetId } = event;
