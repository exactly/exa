...
export default new Hono().get(
  ...
  async (c) => {
    ...
    if (maturity && c.req.header("accept") === "application/pdf") {
      ...
      try {
        const pdf = await startSpan({ name: "pdf.render.statement.head", op: "pdf.render.statement.head" }, () =>
          renderToBuffer(Statement({ ...statement, data: [] })),
        );
        const buffer = startSpan({ name: "pdf.to.buffer.head", op: "pdf.to.buffer.head" }, () =>
          toArrayBuffer(pdf.buffer),
        );
        //return c.body(buffer, 200, { "content-type": "application/pdf" });
      } catch (error) {
        captureException(error, {
          level: "error",
          contexts: { statement: { ...statement, data: [] } },
        });
      }
