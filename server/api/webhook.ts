...
export default new Hono()
  .get(
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": {
              schema: resolver(record(string(), BaseWebhook), { errorMode: "ignore" }),
            },
          },
        },
        401: {
          ...
          content: {
            "application/json": {
              schema: resolver(
                union([
                  object({
                    code: pipe(literal("no organization"), metadata({ examples: ["no organization"] })),
                  }),
                  object({
                    code: pipe(literal("no permission"), metadata({ examples: ["no permission"] })),
                  }),
                ]),
                { errorMode: "ignore" },
              ),
    ...
    async (c) => {
      ...
      const config = parse(
        object({
          ...WebhookConfig.entries,
          webhooks: record(string(), BaseWebhook),
        }),
        source?.config,
      );
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": {
              schema: resolver(Webhook, { errorMode: "ignore" }),
            },
          },
        },
        401: {
      ...
      return mutex
        .runExclusive(async () => {
          ...
          if (source) {
            ...
            const webhook = {
              ...payload,
              secret: config.webhooks[name]?.secret ?? randomBytes(16).toString("hex"),
            };
            await database
              .update(sources)
              .set({
    ...
    describeRoute({
      ...
      responses: {
        200: {
          ...
          content: {
            "application/json": {
              schema: resolver(object({ code: literal("ok") }), { errorMode: "ignore" }),
            },
          },
        },
        401: {
