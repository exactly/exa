...
import type { Org } from "../middleware/org";
  ...
  return new Hono()
    .get(
      ...
      async (c) => {
        ...
        const organizations = await betterAuth.api.listOrganizations({ headers: c.req.raw.headers });
        const organizationId = organizations[0]?.id;
        if (!organizationId) return c.json({ code: "no organization" }, 403);

        const { success } = await betterAuth.api.hasPermission({
    ...
    .patch(
      ...
      async (c) => {
        ...
        const id = organizations[0]?.id;
        if (!id) return c.json({ code: "no organization" }, 403);
        const { success: canUpdate } = await betterAuth.api.hasPermission({
          headers: c.req.raw.headers,
    ...
    .delete(
      ...
      async (c) => {
        ...
        const { success: canDelete } = await betterAuth.api.hasPermission({
          headers: c.req.raw.headers,
