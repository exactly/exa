  ...
  return new Hono()
    .get(
      ...
      async (c) => {
        ...
        if ((scope === "business") !== (parse(Address, credential.salt) === parse(Address, businessSalt))) {
...
Business requests require a business credential whose salt matches the configured business salt; the \`account-type\` request header is not used. Otherwise, the endpoint returns \`400 { "code": "not supported" }\`. The body must be exactly one of two variants:
              ...
              "application/json": {
                schema: resolver(
                  union([
                    CompanyApplicationResponse,
                    CompanyApplicationStatusResponse,
                    object({ kycLink: optional(pipe(string(), url())), status: string() }),
                  ]),
                  { errorMode: "ignore" },
                ),
              },
      ...
      vValidator(
        ...
        optional(
          pipe(
            ...
            transform(({ "do-connecting-ip": ip, "x-forwarded-for": forwarded, ...headers }) => ({
              ...headers,
          ...
          return withMutex(account, async () => {
            ...
            try {
              if (payload.scope === "bridge") {
                ...
                return c.json(
                  {
                    kycLink: await bridge.getKYCLink(customer.id, payload.redirectURL).then(async (kycLink) => {
                      await database
                        .update(credentials)
          ...
          return c.json(
            {
              ...
              reason: application.applicationReason ?? "unknown",
              applicationCompletionLink: application.applicationCompletionLink ?? undefined,
              applicationExternalVerificationLink: application.applicationExternalVerificationLink ?? undefined,
            },
            200,
          );
        }
