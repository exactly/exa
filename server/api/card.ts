...
This endpoint only accepts Wallet Extension bearer access. It does not accept \`credential_id\` cookies, Better Auth sessions, or \`sessionid\`.
    ...
    .post(
      ...
      async (c) => {
        const { credentialId } = c.req.valid("cookie");
        const { account: mutexAccount, mutex, release } = await cardMutex(credentialId);
        return mutex
          .runExclusive(async () => {
            const credential = await database.query.credentials.findFirst({
              where: eq(credentials.id, credentialId),
            ...
            try {
              ...
              const productId =
                chain.id === base.id && !mutexAccount && credential.source !== "5lu2sNu0v0ZElC2m77QR3rAZBHLr8PoG" // cspell:ignore azbh
                  ? BASE_PRODUCT_ID
                  : SIGNATURE_PRODUCT_ID;
              const card =
                (await adoptOrphanCard(panda, { credentialId, userId: pandaId })) ??
                (await panda.createCard(pandaId, productId, {
...
`,
      ...
      async (c) => {
        ...
        const { credentialId } = c.req.valid("cookie");
        const { mutex, release } = await cardMutex(credentialId);
        return mutex
          .runExclusive(async () => {
            const credential = await database.query.credentials.findFirst({
              columns: {
