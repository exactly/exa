...
describe("ramp api", () => {
  ...
  describe("limits", () => {
    it("returns 400 if manteca user not started", async () => {
      ...
      const response = await appClient.limits.$get({}, { headers: { "test-credential-id": "ramp-test" } });
    ...
    it("returns limits for active manteca user", async () => {
      vi.spyOn(manteca, "getUser").mockResolvedValue({
        id: "user123",
        numberId: "456",
        status: "ACTIVE",
      ...
      const limits = [
        {
          ...
          yearlyLimit: "1000000",
          availableYearlyLimit: "800000",
          monthlyLimit: "100000",
          availableMonthlyLimit: "80000",
        },
      ];
      vi.spyOn(manteca, "getLimits").mockResolvedValue(limits);

      const response = await appClient.limits.$get({}, { headers: { "test-credential-id": "ramp-test" } });

      expect(response.status).toBe(200);
      await expect(response.json()).resolves.toStrictEqual({ manteca: { limits } });
    });
  });
