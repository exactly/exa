...
describe("authenticated", () => {
  ...
  it("returns panda card provisioning when requested", async () => {
    ...
    expect(json).toStrictEqual({
      ...
      limit: { amount: 5000, frequency: "per24HourPeriod" },
      productId: PLATINUM_PRODUCT_ID,
      provisioning: { processorCardId: "proc-default", timeBasedSecret: "secret-default" },
    });
  ...
  it("returns 404 when provisioning reports stale card", async () => {
    ...
    expect(captureException).toHaveBeenCalledExactlyOnceWith(
      ...
      expect.objectContaining({
        extra: { cardId: "default", credentialId: "default", pandaId: "default", status: "ACTIVE" },
  ...
  it("returns 404 when provisioning reports stale card through parent onError", async () => {
    const error = new ServiceError("Panda", 404, "not found");
    vi.spyOn(panda, "getSecrets").mockResolvedValueOnce(panTemplate);
    vi.spyOn(panda, "getPIN").mockResolvedValueOnce(pinTemplate);
    vi.spyOn(panda, "getCard").mockResolvedValueOnce(cardTemplate);
  ...
  it("returns 404 when provisioning reports stale card and user lookup fails", async () => {
  ...
  it("returns 403 when provisioning reports unapproved user", async () => {
