...
describe("authenticated", () => {
  ...
  it("returns already created for an existing business card without an approved application", async () => {
    const credentialId = "card-business-existing-unapproved";
    const cardId = "00000000-0000-4000-8000-0000000000ab";
    await insertBusinessCredential({
      id: credentialId,
      account: padHex("0x98f", { size: 20 }),
      pandaId: "card-business-existing-unapproved-user",
    });
    await database
      .insert(cards)
    ...
    it("issues a signature product card on base for a business credential", async () => {
      ...
      try {
        ...
        expect(createCard).toHaveBeenCalledExactlyOnceWith("base-business-user", SIGNATURE_PRODUCT_ID, {
          amount: undefined,
          idempotencyKey: "business-approval:base-business:0",
        });
        await expect(response.json()).resolves.toStrictEqual({
          status: "ACTIVE",
          lastFour: "5353",
