...
-----END PUBLIC KEY-----`;
  ...
  describe("cardLimit scope", () => {
    ...
    describe("getting kyc", () => {
      ...
      it("falls back to inquiry check when getCardLimitAccount fails", async () => {
        vi.spyOn(persona, "getCardLimitAccount").mockRejectedValueOnce(new Error("network error"));
        vi.spyOn(persona, "getPendingInquiryTemplate").mockResolvedValueOnce(persona.CARD_LIMIT_TEMPLATE);
      ...
      it("falls back to inquiry check when card_limit_usd is null", async () => {
        vi.spyOn(persona, "getCardLimitAccount").mockResolvedValueOnce({
          id: "acct-1",
          type: "account",
        ...
        expect(persona.getInquiry).toHaveBeenCalledWith("bob", persona.CARD_LIMIT_TEMPLATE);
        await expect(response.json()).resolves.toStrictEqual({ code: "ok" });
        expect(response.status).toBe(200);
      });

      it("returns not started when basic kyc is not approved", async () => {
        vi.spyOn(persona, "getCardLimitAccount").mockResolvedValueOnce(undefined); // eslint-disable-line unicorn/no-useless-undefined
        vi.spyOn(persona, "getPendingInquiryTemplate").mockResolvedValueOnce(persona.PANDA_TEMPLATE);
        const getInquiry = vi.spyOn(persona, "getInquiry");
      ...
      it("returns ok when inquiry is approved", async () => {
        vi.spyOn(persona, "getCardLimitAccount").mockResolvedValueOnce(undefined); // eslint-disable-line unicorn/no-useless-undefined
        vi.spyOn(persona, "getPendingInquiryTemplate").mockResolvedValueOnce(persona.CARD_LIMIT_TEMPLATE);
        vi.spyOn(persona, "getInquiry").mockResolvedValueOnce(personaTemplate);
        vi.mocked(captureException).mockClear();
      ...
      it.each(["created", "expired", "pending"] as const)("returns not started when inquiry is %s", async (status) => {
        ...
        vi.spyOn(persona, "getInquiry").mockResolvedValueOnce({
          ...personaTemplate,
          attributes: { ...personaTemplate.attributes, status },
        });
        const response = await appClient.index.$get(
          { query: { scope: "cardLimit" } },
          { headers: { "test-credential-id": "bob", SessionID: "fakeSession" } },
    ...
    describe("posting kyc", () => {
      it("returns already approved when persona account has card_limit_usd set", async () => {
        ...
        const getPendingInquiryTemplate = vi.spyOn(persona, "getPendingInquiryTemplate");
        const createInquiry = vi.spyOn(persona, "createInquiry");
      ...
      it("falls back to inquiry creation when getCardLimitAccount fails", async () => {
        const sessionToken = "persona-session-token";
      ...
      it("proceeds to inquiry when card_limit_usd is null", async () => {
        const sessionToken = "persona-session-token";
      ...
      it("proceeds to inquiry when card_limit_usd field is absent", async () => {
        const sessionToken = "persona-session-token";
