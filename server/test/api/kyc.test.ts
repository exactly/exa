...
} satisfies ReturnType<typeof createAllow>;
const credit = {
  close: vi.fn(() => Promise.resolve()),
  enqueue: vi.fn(() => Promise.resolve()),
...
describe("authenticated", () => {
  ...
  describe("application", () => {
    describe("with organization", () => {
      ...
      describe("submit", () => {
        const businessId = "bob-business-individual";
        const businessAccount = getAddress(padHex("0xb0c", { size: 20 }));
        ...
        it("returns bad request when a business credential uses the individual application", async () => {
          const statement = `I apply for KYC approval on behalf of address ${businessAccount} with payload hash ${sha256(Buffer.from(canonicalize(applicationPayload) ?? "", "utf8"))}`;
          const message = createSiweMessage({
            statement,
            resources: ["https://exactly.github.io/exa"],
            nonce: generateSiweNonce(),
...
-----END PUBLIC KEY-----`;
    ...
    describe("business application", () => {
      ...
      function postApplication(header: Record<string, string> = {}) {
        return appClient.application.$post(
          { json: { scope: "panda" } },
          {
            headers: {
              "test-credential-id": businessId,
      ...
      it("routes on the credential salt, ignoring the account type header", async () => {
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue(pendingApplication);

        const response = await postApplication({ "Account-Type": "corporate" });
      ...
      it("submits a company application for a business credential", async () => {
        ...
        const response = await postApplication({ "do-connecting-ip": "127.0.0.1" });
      ...
      it("reads the client ip from the forwarded header", async () => {
        mockProfile();
        const createCompanyApplication = vi
          .spyOn(panda, "createCompanyApplication")
          .mockResolvedValue(pendingApplication);
        ...
        const response = await postApplication({ "x-forwarded-for": "1.2.3.4, 203.0.113.7" });
      ...
      it.each([
        ["", undefined],
        [" with an existing panda id", "panda-1"],
      ])("creates a bridge customer when the business credential%s", async (_state, pandaId) => {
        if (pandaId) await database.update(credentials).set({ pandaId }).where(eq(credentials.id, businessId));
        mockProfile();
        ...
        expect(updatedCredential).toMatchObject({ bridgeId: "customer-1", pandaId: pandaId ?? null });
      ...
      it("creates a bridge customer from the business name and email alone", async () => {
        mockProfile({ collected_email_address: "jane@example.com", i_company_name: "Account Acme" });
        const createCustomer = vi
          .spyOn(bridge, "createCustomer")
          .mockResolvedValue({ id: "customer-1", status: "awaiting_ubo" });
      ...
      it("returns a bad request when a Bridge application has no terms", async () => {
        const createCustomer = vi.spyOn(bridge, "createCustomer");

        const response = await appClient.application.$post({ json: { scope: "bridge" } }, { headers: businessHeaders });
      ...
      it("returns a bad request when a Bridge application has empty terms", async () => {
        const createCustomer = vi.spyOn(bridge, "createCustomer");

        const response = await appClient.application.$post(
          { json: { acceptedTermsId: "", scope: "bridge" } },
          { headers: businessHeaders },
        );
      ...
      it("returns an existing company application without a status", async () => {
        const companyApplication = { ...pendingApplication, applicationStatus: null };
        const getCompanyApplication = vi.spyOn(panda, "getCompanyApplication").mockResolvedValue(companyApplication);
      ...
      it("returns a business application error", async () => {
        const error = new BusinessApplicationError("business account is not complete", "processing");
        const businessApplication = vi.spyOn(panda, "businessApplication").mockRejectedValueOnce(error);
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue(undefined); // eslint-disable-line unicorn/no-useless-undefined

        const response = await postApplication({ "do-connecting-ip": "127.0.0.1" });

        expect(response.status).toBe(400);
        expect(businessApplication).toHaveBeenCalledOnce();
        await expect(response.json()).resolves.toStrictEqual({
          code: "processing",
          message: ["business account is not complete"],
        });
      ...
      it("returns no credential when the business credential disappears", async () => {
        const credential = await database.query.credentials.findFirst({ where: eq(credentials.id, businessId) });
        const findFirst = vi
          .spyOn(database.query.credentials, "findFirst")
      ...
      it("finalizes an approved company application", async () => {
        ...
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...
          type: "account",
          attributes: { fields: {} },
        } as Persona.AccountOutput<"cardLimit">);
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue(undefined); // eslint-disable-line unicorn/no-useless-undefined
      ...
      it("sets sentry user before business finalization failures", async () => {
        vi.mocked(captureException).mockClear();
        ...
        vi.spyOn(panda, "getCards").mockResolvedValue([]);
        const error = new Error("persona unavailable");
        vi.spyOn(persona, "getAccount").mockRejectedValue(error);
        ...
        expect(captureException).toHaveBeenCalledWith(
          error,
          expect.objectContaining({ contexts: { details: { credentialId: businessId, scope: "cardLimit" } } }),
        );
        expect(setUser).toHaveBeenCalledWith({ id: businessAccount });
        expect(vi.mocked(setUser).mock.invocationCallOrder[0]).toBeLessThan(
      ...
      it("retries credit when an approved business card already exists", async () => {
        ...
        const response = await postApplication({ "account-type": "business" });
      ...
      it("returns bad request when a business scope payload includes a verify payload", async () => {
        const response = await appClient.application.$post(
          {
            json: {
              scope: "panda",
      ...
      it("returns bad request when an individual credential uses the business payload", async () => {
        const response = await appClient.application.$post(
          { json: { scope: "panda" } },
          { headers: { "test-credential-id": "bob", SessionID: "fakeSession" } },
        );
        ...
        expect(response.status).toBe(400);
        await expect(response.json()).resolves.toStrictEqual({ code: "bad request", legacy: "bad request" });
      });

      it("returns not supported for an unaccepted business application scope", async () => {
        const businessApplication = vi.spyOn(panda, "businessApplication");
        const createCompanyApplication = vi.spyOn(panda, "createCompanyApplication");
      ...
      it("finalizes an approved company when the status is polled", async () => {
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue({
          id: "company-approved",
          applicationStatus: "approved",
          applicationReason: "",
        ...
        const getCompanyUsers = vi
          ...
          .mockResolvedValue([{ id: "business-user", walletAddress: businessAccount }]);
        vi.spyOn(panda, "getCards").mockResolvedValue([]);
        vi.spyOn(persona, "getAccount").mockResolvedValue(undefined); // eslint-disable-line unicorn/no-useless-undefined
        const createCard = vi.spyOn(panda, "createCard").mockResolvedValue({
          id: "polled-card",
        ...
        expect(getCompanyUsers).toHaveBeenCalledExactlyOnceWith("company-approved");
        expect(createCard).toHaveBeenCalledWith("business-user", SIGNATURE_PRODUCT_ID, expect.anything());
        expect(allow.enqueue).toHaveBeenCalledExactlyOnceWith(
          expect.objectContaining({ account: businessAccount, salt: businessSalt }),
        );
        expect(credit.enqueue).toHaveBeenCalledExactlyOnceWith(
          businessAccount,
          "business-approval:business-kyc:polled-card",
        );
        await expect(
          database.query.credentials.findFirst({ columns: { pandaId: true }, where: eq(credentials.id, businessId) }),
        ).resolves.toMatchObject({ pandaId: "business-user" });
      });

      it("retries credit when polling an approved company that already has an active card", async () => {
        await database.update(credentials).set({ pandaId: "business-user" }).where(eq(credentials.id, businessId));
        ...
        expect(credit.enqueue).toHaveBeenCalledExactlyOnceWith(
          businessAccount,
          "business-approval:business-kyc:business-poll-card",
        );
      });

      it("finalizes an approved company once when a webhook finalization is concurrent", async () => {
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue({
          id: "company-approved-race",
          applicationStatus: "approved",
          applicationReason: "",
        });
        const getCompanyUsers = vi
        ...
        const webhook = Panda.withMutex(businessAccount, async () => {
          await new Promise((resolve) => setTimeout(resolve, 100));
          await Panda.finalizeApproval(businessId, "company-approved-race", businessAccount, database, panda, {
            allow,
            credit,
            persona,
            sardine: createSardine("sardine", "https://sardine.test"),
        ...
        await vi.waitUntil(() => Panda.getMutex(businessAccount) !== undefined, 26_666);
        const poll = await appClient.application.$get(
          { query: {} },
          { headers: { "test-credential-id": businessId, SessionID: "fakeSession" } },
        );
        await webhook;
        ...
        expect(getCompanyUsers).toHaveBeenCalledTimes(1);
      });

      it("returns company application verification links", async () => {
        const completionLink = {
          url: "https://cardmemberportal.com/completion",
          params: { userId: "0e3c467c-01e3-4fe8-8778-1c88e02fd000", signature: "y".repeat(156) },
        };
        const verificationLink = {
          url: "https://cardmemberportal.com/kyc",
          params: { userId: "0e3c467c-01e3-4fe8-8778-1c88e02fd000", signature: "x".repeat(156) },
        };
        vi.spyOn(panda, "getCompanyApplication").mockResolvedValue({
          id: businessId,
      ...
      it("returns bad kyb when the company application is denied", async () => {
        ...
        expect(response.status).toBe(400);
        expect(getCompanyApplication).toHaveBeenCalledWith(businessId);
        await expect(response.json()).resolves.toStrictEqual({ code: "bad kyb" });
      });
