...
-----END PUBLIC KEY-----`;
    ...
    describe("business application", () => {
      ...
      it("reads the client ip from the forwarded header", async () => {
        ...
        const response = await appClient.application.$post({ json: { scope: "panda" } }, { headers: businessHeaders });
      ...
      it.each([
        ...
        expect(response.status).toBe(200);
        expect(businessApplication.mock.calls.at(-1)?.[2]).toBe("203.0.113.7");
        ...
        expect(getKYCLink).toHaveBeenCalledWith("customer-1");
