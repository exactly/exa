...
const bullmq = { on: mocks.on } as unknown as Redis;
...
describe("chat hook", () => {
  ...
  beforeEach(async () => {
    ...
    mocks.on.mockReset();
    mocks.quit.mockReset().mockResolvedValue();
  ...
  it("accepts unsigned webhooks when no secret is configured", async () => {
    const unsignedHook = createChatHook({ bullmq, close: mocks.quit, whatsappFrom: "321" });
    try {
      await unsignedHook.ready;
      const response = await unsignedHook.app.request("/", {
        method: "POST",
        body: JSON.stringify(message()),
        ...
        changes: [
          {
            ...
            value: {
              ...
              contacts: [{ profile: { name: "Jhon" }, user_id: "US.12345678" }], // cspell:ignore Jhon
