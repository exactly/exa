...
describe("address activity", () => {
  ...
  it("calls pokeAccountAssets with correct ignore option", async () => {
    const pokeAccountAssetsSpy = vi.spyOn(persona, "pokeAccountAssets").mockResolvedValue();
    ...
    await vi.waitUntil(() => pokeAccountAssetsSpy.mock.calls.length > 0, { timeout: 5000 });

    expect(pokeAccountAssetsSpy).toHaveBeenCalledWith(account, { ignore: [`NotAllowed(${account})`] });
  });
});
