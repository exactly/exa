...
describe("webhooks", () => {
  ...
  it("adopts the company user and issues the card for an approved company", async () => {
    ...
    await database.insert(credentials).values({
      ...
      salt: accountSalt("business"),
    });
    vi.spyOn(panda, "getCompany").mockResolvedValue({ externalId: credentialId });
  ...
  it.each([
    { provider: {}, slug: "missing" },
    { provider: { externalId: "unknown-business-credential" }, slug: "unknown" },
  ] satisfies { provider: { externalId?: string }; slug: string }[])(
    "returns a retryable error when the approved company mapping is $slug",
    async ({ provider, slug }) => {
      vi.spyOn(panda, "getCompany").mockResolvedValue(provider);
      const getCompanyUsers = vi.spyOn(panda, "getCompanyUsers");
      const createCard = vi.spyOn(panda, "createCard");
      const response = await appClient.index.$post({
        header: { signature: "panda-signature" },
        json: {
          id: `company-mapping-${slug}`,
          resource: "company",
          action: "updated",
          body: { id: `business-company-mapping-${slug}`, applicationStatus: "approved" },
        },
      });
