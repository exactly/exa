...
describe("webhooks", () => {
  ...
  it("adopts the company user and issues the card for an approved company", async () => {
    ...
    try {
      ...
      expect(createCard).toHaveBeenCalledExactlyOnceWith("business-user", SIGNATURE_PRODUCT_ID, {
        amount: undefined,
        idempotencyKey: `business-card:${credentialId}:company-approved`,
      });
  ...
  it("uses one idempotency key for concurrent company approvals", async () => {
    ...
    const payload: Parameters<typeof appClient.index.$post>[0] = {
      header: { signature: "panda-signature" },
      json: {
    ...
    try {
      ...
      for (const call of createCard.mock.calls)
        expect(call[2]).toStrictEqual({
          amount: undefined,
          idempotencyKey: `business-card:${credentialId}:company-approved-concurrent`,
        });
      await expect(
  ...
  it("reuses the stored company user and local card for repeated approvals", async () => {
  ...
  it("issues a replacement card after a deleted card", async () => {
    ...
    try {
      ...
      expect(createCard).toHaveBeenCalledExactlyOnceWith("business-user", SIGNATURE_PRODUCT_ID, {
        amount: undefined,
        idempotencyKey: `business-card:${credentialId}:company-approved-reissue`,
      });
    } finally {
      await database.delete(cards).where(eq(cards.credentialId, credentialId));
      await database.delete(credentials).where(eq(credentials.id, credentialId));
