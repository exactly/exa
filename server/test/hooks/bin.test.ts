...
const mocks = {
  ...
  supervise: vi.fn<(name: string, created: Promise<Hook>, app?: Hono) => void>(),
};
...
describe("activity bin", () => {
  it("resolves private config before constructing and supervising the hook", async () => {
    ...
    const response = await app.request("/hooks/activity");
...
async function load() {
  await import("../../hooks/activity/bin");
  const created = mocks.supervise.mock.calls[0]?.[1];
  if (!created) throw new Error("missing activity hook");
  await created;
  const app = mocks.supervise.mock.calls[0]?.[2];
  if (!app) throw new Error("missing activity app");
  expect(mocks.supervise).toHaveBeenCalledExactlyOnceWith("activity", created, app);
  return app;
}
