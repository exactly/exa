...
import { ErrorCodes } from "../../utils/ramps/bridge";
...
vi.mock("../../database", () => ({
  default: { update: vi.fn(() => ({ set: vi.fn(() => ({ where: vi.fn() })) })) },
  credentials: {},
}));
...
describe("bridge utils", () => {
  ...
  describe("getCustomer", () => {
    it("returns customer when found", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse(mockActiveCustomer));

      const result = await bridge.getCustomer("cust-123");

      expect(result).toStrictEqual(mockActiveCustomer);
    });

    it("returns undefined when not found", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchError(404, "not_found"));
  ...
  describe("getQuote", () => {
    it("returns transformed quote", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ midmarket_rate: "1.00", buy_rate: "0.99", sell_rate: "1.01" }),
      );
  ...
  describe("createCustomer", () => {
    it("returns new customer", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse({ id: "cust-new", status: "not_started" }));

      const result = await bridge.createCustomer(mockCreateCustomerPayload);
    ...
    it("throws EMAIL_ALREADY_EXISTS when email is taken", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchError(409, "A customer with this email already exists"),
      );

      await expect(bridge.createCustomer(mockCreateCustomerPayload)).rejects.toThrow(ErrorCodes.EMAIL_ALREADY_EXISTS);
    ...
    it("throws INVALID_ADDRESS when residential_address is invalid", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchError(400, "invalid_parameters: residential_address is not valid"),
      );

      await expect(bridge.createCustomer(mockCreateCustomerPayload)).rejects.toThrow(ErrorCodes.INVALID_ADDRESS);
    ...
    it("throws on other errors", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchError(400, "bad request"));

      await expect(bridge.createCustomer(mockCreateCustomerPayload)).rejects.toThrow("bad request");
    });
  });
  ...
  describe("getProvider", () => {
    ...
    describe("with existing customer", () => {
      it("throws when customer not found", async () => {
        ...
        await expect(bridge.getProvider({ credentialId: "cred-1", customerId: "cust-bad" })).rejects.toThrow(
          ErrorCodes.BAD_BRIDGE_ID,
        );
      });

      it("returns NOT_AVAILABLE when customer is offboarded", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({ ...mockActiveCustomer, status: "offboarded" }),
        );
      ...
      it("returns NOT_AVAILABLE when customer is rejected", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({ ...mockActiveCustomer, status: "rejected" }),
        );
      ...
      it("returns NOT_AVAILABLE when customer is paused", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({ ...mockActiveCustomer, status: "paused" }),
        );
      ...
      it("returns ONBOARDING when customer is under_review", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({ ...mockActiveCustomer, status: "under_review" }),
        );
      ...
      it("returns ONBOARDING when customer is incomplete", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({ ...mockActiveCustomer, status: "incomplete" }),
        );
      ...
      it("returns ACTIVE with GBP from faster_payments endorsement", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({
            ...mockActiveCustomer,
            endorsements: [mockEndorsement("base", "approved"), mockEndorsement("faster_payments", "approved")],
          }),
        );
      ...
      it("returns ACTIVE with currencies from approved endorsements", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({
            ...mockActiveCustomer,
            endorsements: [mockEndorsement("base", "approved"), mockEndorsement("sepa", "approved")],
          }),
        );
      ...
      it("stops collecting currencies on first non-approved endorsement", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({
            ...mockActiveCustomer,
            endorsements: [mockEndorsement("base", "approved"), mockEndorsement("sepa", "incomplete")],
          }),
        );
      ...
      it("captures exception for additional requirements on endorsement", async () => {
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
          mockFetchResponse({
            ...mockActiveCustomer,
            endorsements: [{ ...mockEndorsement("base", "approved"), additional_requirements: ["tos_acceptance"] }],
          }),
        );
    ...
    describe("without existing customer", () => {
      it("throws when persona account not found", async () => {
        ...
        await expect(bridge.getProvider({ credentialId: "cred-1" })).rejects.toThrow(ErrorCodes.NO_PERSONA_ACCOUNT);
      });

      it("throws when no valid document found", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce(mockPersonaAccountBridge);
        vi.spyOn(persona, "getValidDocumentForBridge").mockReturnValueOnce(undefined); // eslint-disable-line unicorn/no-useless-undefined

        await expect(bridge.getProvider({ credentialId: "cred-1" })).rejects.toThrow(ErrorCodes.NO_DOCUMENT);
      });

      it("returns NOT_AVAILABLE when id class is not mappable to bridge type", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce(mockPersonaAccountBridge);
        vi.spyOn(persona, "getValidDocumentForBridge").mockReturnValueOnce({
      ...
      it("throws when country alpha3 conversion fails", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...mockPersonaAccountBridge,
          attributes: { ...mockPersonaAccountBridge.attributes, "country-code": "INVALID" },
        });

        await expect(bridge.getProvider({ credentialId: "cred-1" })).rejects.toThrow(ErrorCodes.NO_COUNTRY_ALPHA3);
      });

      it("throws when US user has no SSN", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...mockPersonaAccountBridge,
          attributes: { ...mockPersonaAccountBridge.attributes, "country-code": "US", "social-security-number": null },
        });

        await expect(bridge.getProvider({ credentialId: "cred-1" })).rejects.toThrow(
          ErrorCodes.NO_SOCIAL_SECURITY_NUMBER,
        );
      });

      it("returns NOT_STARTED with basic currencies for standard country", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce(mockPersonaAccountBridge);
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse({ url: "https://tos.link" }));
      ...
      it("appends spei endorsement for MX country", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...mockPersonaAccountBridge,
          attributes: { ...mockPersonaAccountBridge.attributes, "country-code": "MX" },
        });
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse({ url: "https://tos.link" }));
      ...
      it("appends pix endorsement for BR country", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...mockPersonaAccountBridge,
          attributes: { ...mockPersonaAccountBridge.attributes, "country-code": "BR" },
        });
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse({ url: "https://tos.link" }));
      ...
      it("appends faster_payments endorsement for GB country", async () => {
        vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
          ...mockPersonaAccountBridge,
          attributes: { ...mockPersonaAccountBridge.attributes, "country-code": "GB" },
        });
        vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockFetchResponse({ url: "https://tos.link" }));
      ...
      it("appends redirect URL with provider param", async () => {
        ...
        const fetchSpy = vi
          ...
          .mockResolvedValueOnce(mockFetchResponse({ url: "https://tos.link" }));
  ...
  describe("onboarding", () => {
    it("throws ALREADY_ONBOARDED when customerId exists", async () => {
      ...
      ).rejects.toThrow(ErrorCodes.ALREADY_ONBOARDED);
    });
    ...
    it("throws NOT_SUPPORTED_CHAIN_ID for unsupported chain", async () => {
      ...
      ).rejects.toThrow(ErrorCodes.NOT_SUPPORTED_CHAIN_ID);
      expect(captureException).toHaveBeenCalledWith(
        expect.objectContaining({ message: "bridge not supported chain id" }),
        expect.objectContaining({ level: "error" }),
      );
    ...
    it("throws when persona account not found", async () => {
      ...
      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NO_PERSONA_ACCOUNT);
    });
    ...
    it("throws when no valid document found", async () => {
      ...
      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NO_DOCUMENT);
    });
    ...
    it("throws when front document photo is missing", async () => {
      ...
      vi.spyOn(persona, "getDocument").mockResolvedValueOnce({
        ...
        attributes: { ...mockDocumentResponse.attributes, "front-photo": null },
      });

      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NO_DOCUMENT_FILE);
    });
    ...
    it("throws when id class is not mappable to bridge type", async () => {
      vi.spyOn(persona, "getAccount").mockResolvedValueOnce({
        ...mockPersonaAccountBridge,
        attributes: {
          ...mockPersonaAccountBridge.attributes,
          fields: {
            ...mockPersonaAccountBridge.attributes.fields,
            documents: { value: [{ value: { ...mockIdentityDocument, id_class: { value: "wp" } } }] },
          },
        },
      });
      vi.spyOn(persona, "getDocument").mockResolvedValueOnce(mockDocumentResponse);
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(mockBlobResponse()).mockResolvedValueOnce(mockBlobResponse());

      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NOT_FOUND_IDENTIFICATION_CLASS);
    });
    ...
    it("throws when country alpha3 conversion fails", async () => {
      ...
      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NO_COUNTRY_ALPHA3);
    });
    ...
    it("throws when US user has no SSN", async () => {
      ...
      await expect(
        bridge.onboarding({ credentialId: "cred-1", customerId: null, acceptedTermsId: "terms-1" }),
      ).rejects.toThrow(ErrorCodes.NO_SOCIAL_SECURITY_NUMBER);
    });
    ...
    it("includes ssn for US country", async () => {
      ...
      const fetchSpy = vi
        ...
        .mockResolvedValueOnce(mockFetchResponse({ id: "cust-new", status: "not_started" }));
  ...
  describe("getDepositDetails", () => {
    ...
    it("throws NOT_SUPPORTED_CHAIN_ID for unsupported chain", async () => {
      ...
      await expect(bridge.getDepositDetails("USD", account, mockActiveCustomer)).rejects.toThrow(
        ErrorCodes.NOT_SUPPORTED_CHAIN_ID,
      );
    });

    it("throws NOT_ACTIVE_CUSTOMER when customer is not active", async () => {
      await expect(
        bridge.getDepositDetails("USD", account, { ...mockActiveCustomer, status: "under_review" }),
      ).rejects.toThrow(ErrorCodes.NOT_ACTIVE_CUSTOMER);
    });

    it("throws NOT_AVAILABLE_CURRENCY when currency is not endorsed", async () => {
      await expect(bridge.getDepositDetails("EUR", account, mockActiveCustomer)).rejects.toThrow(
        ErrorCodes.NOT_AVAILABLE_CURRENCY,
      );
    });

    it("returns USD deposit details from existing virtual account", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ count: 1, data: [mockUsdVirtualAccount(account)] }),
      );
    ...
    it("creates virtual account when none exists", async () => {
      vi.spyOn(globalThis, "fetch")
        .mockResolvedValueOnce(mockFetchResponse({ count: 0, data: [] }))
        .mockResolvedValueOnce(mockFetchResponse(mockUsdVirtualAccount(account)));
    ...
    it("returns EUR deposit details with SEPA info", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ count: 1, data: [mockEurVirtualAccount(account)] }),
      );
    ...
    it("returns MXN deposit details with SPEI info", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ count: 1, data: [mockMxnVirtualAccount(account)] }),
      );

      const customer = {
        ...mockActiveCustomer,
        endorsements: [mockEndorsement("base", "approved"), mockEndorsement("spei", "approved")],
      };
    ...
    it("returns GBP deposit details with Faster Payments info", async () => {
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ count: 1, data: [mockGbpVirtualAccount(account)] }),
      );
    ...
    it("throws INVALID_ACCOUNT when virtual account destination does not match", async () => {
      ...
      vi.spyOn(globalThis, "fetch").mockResolvedValueOnce(
        mockFetchResponse({ count: 1, data: [mockUsdVirtualAccount(wrongAccount)] }),
      );
  ...
  describe("getVirtualAccounts", () => {
    it("paginates when count exceeds first page", async () => {
      const page1 = Array.from({ length: 20 }, (_, index) => ({
        ...mockUsdVirtualAccount(`0x${String(index).padStart(40, "0")}`),
        id: `va-${String(index)}`,
      }));
      const page2 = [{ ...mockUsdVirtualAccount("0x20"), id: "va-20" }];
      vi.spyOn(globalThis, "fetch")
        .mockResolvedValueOnce(mockFetchResponse({ count: 21, data: page1 }))
        .mockResolvedValueOnce(mockFetchResponse({ count: 21, data: page2 }));
    ...
    it("does not paginate when all results fit in first page", async () => {
      const fetchSpy = vi
        .spyOn(globalThis, "fetch")
        .mockResolvedValueOnce(mockFetchResponse({ count: 1, data: [mockUsdVirtualAccount("0x1")] }));
  ...
  describe("getCryptoDepositDetails", () => {
    ...
    it("throws NOT_SUPPORTED_CHAIN_ID for unsupported chain", async () => {
      ...
      await expect(bridge.getCryptoDepositDetails("USDT", "TRON", account, mockActiveCustomer)).rejects.toThrow(
        ErrorCodes.NOT_SUPPORTED_CHAIN_ID,
      );
    });
    ...
    it("throws NOT_ACTIVE_CUSTOMER when customer is not active", async () => {
      await expect(
        bridge.getCryptoDepositDetails("USDT", "TRON", account, { ...mockActiveCustomer, status: "rejected" }),
      ).rejects.toThrow(ErrorCodes.NOT_ACTIVE_CUSTOMER);
    });

    it("throws NOT_AVAILABLE_CRYPTO_PAYMENT_RAIL for invalid combination", async () => {
      await expect(bridge.getCryptoDepositDetails("USDC", "TRON", account, mockActiveCustomer)).rejects.toThrow(
        ErrorCodes.NOT_AVAILABLE_CRYPTO_PAYMENT_RAIL,
      );
    });
    ...
    it("returns TRON deposit details from existing liquidation address", async () => {
      ...
      const result = await bridge.getCryptoDepositDetails("USDT", "TRON", account, mockActiveCustomer);
    ...
    it("returns SOLANA deposit details", async () => {
      ...
      const result = await bridge.getCryptoDepositDetails("USDC", "SOLANA", account, mockActiveCustomer);
    ...
    it("returns STELLAR deposit details", async () => {
      ...
      const result = await bridge.getCryptoDepositDetails("USDC", "STELLAR", account, mockActiveCustomer);
...
const mockPersonaAccountBridge = {
  id: "account-123",
...
const mockActiveCustomer = {
  ...
  endorsements: [] as ReturnType<typeof mockEndorsement>[],
};

const activeCustomerWithBaseEndorsement = {
  ...mockActiveCustomer,
  endorsements: [mockEndorsement("base", "approved")],
};

const activeCustomerWithSepaEndorsement = {
  ...mockActiveCustomer,
  endorsements: [mockEndorsement("sepa", "approved")],
};

const activeCustomerWithFasterPaymentsEndorsement = {
  ...mockActiveCustomer,
  endorsements: [mockEndorsement("faster_payments", "approved")],
};

function mockUsdVirtualAccount(account: string) {
  return {
    id: "va-usd",
    status: "activated",
...
function mockEurVirtualAccount(account: string) {
  return {
    id: "va-eur",
    status: "activated",
    source_deposit_instructions: {
...
function mockMxnVirtualAccount(account: string) {
  return {
    id: "va-mxn",
    status: "activated",
    source_deposit_instructions: {
...
function mockGbpVirtualAccount(account: string) {
  return {
    id: "va-gbp",
    status: "activated",
    source_deposit_instructions: {
...
const mockCreateCustomerPayload = {
