...
function insertAccounts(accounts: { account: Address }[]) {
...
describe("worker", () => {
  ...
  it("processes check-debts job with debt in single market", async () => {
    ...
    expect(sendPushNotification).toHaveBeenCalledWith({
      ...
      contents: {
        ...
        es: "Tu deuda vence en 24 horas. Repágala ahora para evitar la liquidación.", // cspell:ignore Repágala deuda vence ahora evitar liquidación
  ...
  it("removes orphan reminder when sibling enqueue fails", async () => {
    const now = Math.floor(Date.now() / 1000);
    const nextMaturity = now - (now % MATURITY_INTERVAL) + MATURITY_INTERVAL;
    vi.spyOn(Date, "now").mockReturnValue((nextMaturity - 25 * 3600) * 1000);
    try {
      ...
      await expect(scheduleMaturityChecks()).rejects.toThrow("redis down");
      addSpy.mockRestore();
      const jobs = await queue.getJobs(["delayed", "waiting"]);
      expect(jobs.filter((job) => job.data.maturity === nextMaturity)).toHaveLength(0);
    } finally {
      vi.mocked(Date.now).mockRestore();
    }
  });
  ...
  it("claims the notification marker before sending push", async () => {
    const account = parse(Address, "0x1234567890123456789012345678901234567890");
    const maturity = Math.floor(Date.now() / 1000) + 1800;
    const window = "24h";
    ...
    expect(sendPushNotification).toHaveBeenCalledOnce();
    expect(setSpy.mock.invocationCallOrder[0]).toBeLessThan(sendPushNotification.mock.invocationCallOrder[0] ?? 0);
    const key = `notification:sent:${account}:${String(maturity)}:${window}`;
    await expect(redis.get(key)).resolves.not.toBeNull();
  });
  ...
  it("reschedules after processing any window", async () => {
    ...
    expect(jobs).toHaveLength(2);
    let windows = jobs.map((index) => ({ maturity: index.data.maturity, window: index.data.window }));
    ...
    expect(jobs).toHaveLength(2);
    windows = jobs.map((index) => ({ maturity: index.data.maturity, window: index.data.window }));
    expect(windows).toContainEqual({ maturity: expectedNextMaturity, window: "24h" });
    expect(windows).toContainEqual({ maturity: expectedNextMaturity, window: "1h" });
  });
