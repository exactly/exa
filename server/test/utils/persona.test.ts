...
describe("businessProfile", () => {
  const businessFields = {
    business_name_2: { value: "Account Acme" },
  ...
  it("trims a padded required field", async () => {
    mockInquiry();
    mockAccount({ fields: { ...businessFields, business_name_2: { value: " Account Acme " } } });
  ...
  it.each([["business_name_2"], ["collected_email_address"]] as const)("rejects a missing %s", async (name) => {
    mockInquiry();
  ...
  it("rejects a business field with the wrong type", async () => {
    mockInquiry();
    mockAccount({ fields: { ...businessFields, business_name_2: { value: 123 } } });
