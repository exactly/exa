...
describe("businessProfile", () => {
  const businessFields = {
    ...
    company_name: { value: "Account Acme" },
  };
  let fetchSpy: MockInstance<typeof fetch>;
  beforeEach(() => {
    fetchSpy = vi.spyOn(globalThis, "fetch");
  ...
  it("trims a padded required field", async () => {
    mockInquiry();
    mockAccount({ fields: { ...businessFields, company_name: { value: " Account Acme " } } });
  ...
  it.each([["company_name"], ["collected_email_address"]] as const)("rejects a missing %s", async (name) => {
    mockInquiry();
  ...
  it("rejects a business field with the wrong type", async () => {
    mockInquiry();
    mockAccount({ fields: { ...businessFields, company_name: { value: 123 } } });
