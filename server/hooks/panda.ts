            ...
            await startSpan(
              {
                ...
                attributes: {
                  ...
                  timestamp: String(payload.body.spend.timestamp ?? 0),
              ...
              (span) => {
                return verifyPandaSignature({
                  account,
                  amount: -refundAmount,
                  timestamp: payload.body.spend.timestamp ?? 0,
                  signature: v.parse(Hex, payload.body.spend.signature),
                }).then((valid) => {
...
const declineReasons: Record<string, { notify: boolean; reason: string }> = {
  ...
  frozen_card: { reason: "frozen card" as const, notify: true },
