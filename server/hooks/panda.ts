...
import createPanda, { collectors, createMutex, getMutex } from "../utils/panda";
    ...
    async (c) => {
      ...
      switch (payload.action) {
        case "requested": {
          ...
          try {
            const { amount, call, transaction } = await prepareCollection(card, payload, database, panda, wallet);
            const authorize = () => {
        ...
        case "updated":
          if (
            ...
            const signature = await panda.signIssuerOp({ account, amount: -refundAmount, timestamp }); // TODO replace with payload signature
        ...
        case "created": {
          ...
          try {
            const { call } = await prepareCollection(card, payload, database, panda, wallet);
            if (!call) {
              const tx = await database.query.transactions.findFirst({
                where: and(eq(transactions.id, payload.body.id), eq(transactions.cardId, payload.body.spend.cardId)),
              });
...
async function prepareCollection(
  ...
  const call = await (async () => {
    ...
    const signature = await panda.signIssuerOp({ account, amount, timestamp }); // TODO replace with payload signature
    if (payload.body.spend.signature) {
      await startSpan(
        {
          name: "panda.signature",
