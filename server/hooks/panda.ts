      ...
      if (payload.resource !== "transaction") {
        ...
        if (payload.resource === "company") {
          ...
          const credential = await database.query.credentials.findFirst({
            columns: { account: true, factory: true, id: true, publicKey: true, salt: true },
          ...
          if (salt !== v.parse(Address, zeroAddress)) {
            ...
            await withMutex(account, async () => {
              ...
              const localCard = await database.query.cards.findFirst({
                columns: { id: true },
                where: and(eq(cards.credentialId, credential.id), inArray(cards.status, activeStatuses)),
              });
              ...
              if (!userId) {
                ...
                if (current) userId = current.pandaId;
              }
              ...
              if (orphan)
                captureException(new Error("orphan card adopted"), {
                  ...
                  extra: { credentialId: credential.id, pandaId: userId, cardId: orphan.id },
                });
              const card =
                orphan ??
                (await panda.createCard(userId, SIGNATURE_PRODUCT_ID, {
