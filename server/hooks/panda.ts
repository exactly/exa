    ...
    switch (payload.action) {
      case "requested": {
        ...
        if (card.status === "FROZEN") {
          ...
          rejectTx(account, payload, jsonBody, "frozen_card"); // eslint-disable-line @typescript-eslint/no-floating-promises
        ...
        try {
          ...
          if (error instanceof PandaError) {
            ...
            if (error.message !== "Replay") {
              rejectTx(account, payload, jsonBody, error.message); // eslint-disable-line @typescript-eslint/no-floating-promises
            }
          ...
          rejectTx(account, payload, jsonBody, error instanceof Error ? error.message : "unexpected error"); // eslint-disable-line @typescript-eslint/no-floating-promises
...
function rejectTx(
  ...
  const updatePromise = updateTransactionRecord(transactionId, spend, payload, jsonBody, reason).catch(
    (error: unknown) => {
      captureException(error, { level: "error" });
    },
  );
  ...
  if (NOTIFICATION_TRIGGERING_REASONS.has(reason)) {
    const notificationPromise = sendDeclinedNotification(account, spend, reason).catch((error: unknown) => {
      captureException(error, { level: "error" });
    });
    return Promise.all([updatePromise, notificationPromise]).then(() => undefined);
  }
