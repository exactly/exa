...
supervise(
  ...
  Promise.all([
    ...
    secret("redis-url", secrets)
      ...
      .then(
        (bullmq) =>
          [bullmq, createAllow(bullmq), createCredit(bullmq), createRefund(bullmq), createHook(bullmq)] as const,
      ),
    ...
    ([
      ...
      provider,
      persona,
      [bullmq, allow, credit, refund, webhook],
      sardine,
      segment,
      settler,
    ]) =>
      own(
        ...
        () => segment.close(),
        () =>
          Promise.all([allow.close(), credit.close(), refund.close(), webhook.close()]).finally(() => bullmq.quit()),
      ),
  ),
);
