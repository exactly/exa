...
supervise(
  ...
  Promise.all([
    ...
    secret("redis-url", secrets)
      ...
      .then((bullmq) => [bullmq, createAllow(bullmq), createRefund(bullmq), createHook(bullmq)] as const),
    ...
    ([database, issuer, onesignal, provider, persona, [bullmq, allow, refund, webhook], sardine, segment, settler]) =>
      own(
        panda({
          allow,
          database,
        ...
        () => secrets.close(),
        () => segment.close(),
        () => Promise.all([allow.close(), refund.close(), webhook.close()]).finally(() => bullmq.quit()),
      ),
  ),
);
