    ...
    ((config) =>
      withXcodeProjectBeta(
        ...
        (c) => {
          ...
          const hasBundlePhase = target.props.buildPhases.some(
            (phase) => PBXShellScriptBuildPhase.is(phase) && phase.props.name === walletExtensionBundleScript,
          );
          if (!hasBundlePhase) {
            target.createBuildPhase(PBXShellScriptBuildPhase, {
              ...
              outputPaths: ['"$(CONFIGURATION_BUILD_DIR)/$(UNLOCALIZED_RESOURCES_FOLDER_PATH)/main.jsbundle"'],
              shellPath: "/bin/sh",
              shellScript: `set -e
      ...
      withDangerousMod(config, [
        ...
        (c) => {
          ...
          const updatedContents = injectPodfilePostInstall(
            contents,
            "patchMeaWalletAppDelegateModulemap",
...
function getWalletExtensionPodfilePatch() {
  return `    patchMeaWalletAppDelegateModulemap = lambda do
    ...
    patchWalletExtensionSwiftUILinkage = lambda do
      Dir.glob("#{installer.sandbox.root}/Target Support Files/Pods-ExaWalletExtension/Pods-ExaWalletExtension.*.xcconfig").each do |xcconfig|
        ...
        updated = contents
          .gsub(' -l"RCTSwiftUI"', "")
          .gsub(' -l"RCTSwiftUIWrapper"', "")
          .gsub(' "\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUI"', "")
          .gsub(' "\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUIWrapper"', "")
          .gsub(' "-F\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUI"', "")
          .gsub(' "-F\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUIWrapper"', "")
          .gsub(' -fmodule-map-file="\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUI/RCTSwiftUI.modulemap"', "")
          .gsub(' -Xcc -fmodule-map-file="\${PODS_CONFIGURATION_BUILD_DIR}/RCTSwiftUI/RCTSwiftUI.modulemap"', "")
          .gsub(" -Xcc -Xcc ", " -Xcc ")
        File.write(xcconfig, updated) if updated != contents
      end
      installer.pods_project.targets.each do |target|
        next unless target.name == "Pods-ExaWalletExtension"
        target.dependencies.dup.each do |dependency|
          next unless ["RCTSwiftUI", "RCTSwiftUIWrapper"].include?(dependency.name)
    ...
    patchReactCorePrivacyBundles = lambda do
      installer.pods_project.targets.each do |target|
        next unless target.name.start_with?("React-Core") && target.name.end_with?("-React-Core_privacy")
        podName = target.name.sub(/-React-Core_privacy\\z/, "")
        target.build_configurations.each do |buildConfiguration|
          buildConfiguration.build_settings["PRODUCT_NAME"] = target.name
        end
        Dir.glob("#{installer.sandbox.root}/Target Support Files/Pods-*/Pods-*-resources.sh").each do |script|
          contents = File.read(script)
          updated = contents.gsub(
            "\${PODS_CONFIGURATION_BUILD_DIR}/#{podName}/React-Core_privacy.bundle",
            "\${PODS_CONFIGURATION_BUILD_DIR}/#{podName}/#{target.name}.bundle",
          )
    ...
    patchWalletExtensionSwiftUILinkage.call
