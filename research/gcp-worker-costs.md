...
this historical estimate is based on the repository at 2026-07-28 and public google cloud list prices at that date. it
models the fixed-one-worker baseline before crema autoscaling; the current infrastructure starts pools at zero and
scales them from redis demand. it uses usd, 730 hours per average month, on-demand pricing, no committed-use discount,
and no tax. it estimates the one checked-in `base-sepolia` stack in project `exa-dev`.
...
at the research date, the infrastructure set the location to `us-west1` unless overridden, created five cloud run
worker pools, and defaulted every `manualInstanceCount` to one. every resource override was optional and none was
present in the checked-in stack configuration. the deployment workflow supplied only the image tag at deployment
time.
