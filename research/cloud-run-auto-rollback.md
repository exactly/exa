...
3. make each `cloudrunv2.WorkerPool` depend on its matching preflight job. pulumi can run the five independent
   check-and-deploy paths in parallel, while a failed check leaves that worker's stable pool revision untouched.
...
does not run the matching dependent pool operation. that pool's stable revision is untouched. this gives the useful
...
## deployment boundary: module, not worker fleet

there is a defensible argument for a global barrier: all modules come from one image, so it prevents deliberately
promoting a worker set known to contain one broken entrypoint. it is appropriate only if the whole server image is
genuinely one atomic release and mixed module versions are forbidden.

that is not the deployment model the rest of this repository actually has. the seven cloud run services reconcile
independently; one service can fail its deployment health check after another service has already updated. pulumi
does not roll completed operations back when a sibling resource later fails. a worker-only global barrier therefore
does not provide an atomic server release. it only couples five otherwise separate worker pools, service accounts,
secrets, entrypoints, and queues. [service deployment health check](https://docs.cloud.google.com/run/docs/deploying#disabling_the_deployment_health_check),
[pulumi automatic rollback faq](https://www.pulumi.com/docs/support/faq/infrastructure/#automatic-rollbacks)

the matching-check design is the honest default:

- `allow-check` gates only `allow`, and likewise for the other four pools;
- a broken worker keeps its previous revision while healthy workers and services deploy;
- one unrelated worker cannot block an urgent fix to another worker;
- partial releases and version skew remain possible, so database, redis, and queue contracts must stay backward
  compatible across a rollout. that requirement already exists because the services deploy independently.

if atomicity becomes a real invariant, enforce it across all twelve modules with a preflight for every module before
mutating any of them. even that only gates startup: pulumi can still leave mixed state after a later control-plane
failure. applying an all-or-nothing rule to workers alone is the worst middle ground—extra coupling without atomicity.
...
- put each job in the `dependsOn` list of its matching pool.

the existing workflow performs one `pulumi up`, so this creates five parallel gates:

```text
allow-check     -> allow pool
credit-check    -> credit pool
poke-check      -> poke pool
refund-check    -> refund pool
subscribe-check -> subscribe pool
```
...
keep this as a full pulumi update. a targeted update that selects a worker pool without its matching check job can
bypass the gate.

each gate is safe only within one pulumi update graph. the five pool updates are not transactional. a later cloud run
control-plane failure can still leave a mixed set of successful revisions.
...
order each pool after its matching check, but pulumi explicitly does not automatically roll back completed operations
after a later update error. manual rollback means restoring the old program or configuration and running another
update. a failure can therefore leave different pools on different successful revisions.
...
| five preflight jobs with matching dependencies | yes, per pool | no | one check mode and five job resources | primary worker design |
| five preflight jobs with one global barrier | yes, before any pool update | no | cross-couples every pool | only for a genuinely atomic release |
...
- make each pool depend on its matching job, matching the services' independent deployment boundary;
...
this is preventative rejection, not rollback: if a candidate cannot boot, `pulumi up` fails before changing its pool
and that stable revision keeps running. it deliberately does not promise to detect bad queue logic, incompatible data,
