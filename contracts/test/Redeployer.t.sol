...
import { Redeployer, TargetNonceTooLow } from "../script/Redeployer.s.sol";
...
contract RedeployerTest is ForkTest {
  ...
  function test_recoversNativeETHOnPolygon() external {
    ...
    vm.startPrank(acct("admin"));
    ProposalManager(address(ExaPlugin(payable(address(factory.EXA_PLUGIN()))).proposalManager())).allowTarget(
      receiver, true
    );
    vm.stopPrank();
