...
import { IERC1967 } from "@openzeppelin/contracts-v4/interfaces/IERC1967.sol";
import {
...
contract Redeployer is BaseScript {
  ...
  function deployEXA(address proxy) external {
    ...
    proxyAdmin.upgradeAndCall(
      ITransparentUpgradeableProxy(proxy),
      proxyTransferer,
      abi.encodeCall(ProxyTransferer.transferAndUpgrade, (protocol("ProxyAdmin"), address(exa)))
    );
...
contract ProxyTransferer is IERC1967 {
  error NotProxyAdmin();
  error InvalidAdmin(address admin);
  error InvalidImplementation(address implementation);

  function transferAndUpgrade(address admin, address implementation) external {
    if (msg.sender != StorageSlot.getAddressSlot(ERC1967Utils.ADMIN_SLOT).value) revert NotProxyAdmin();
    if (implementation.code.length == 0) revert InvalidImplementation(implementation);
    if (admin.code.length == 0) revert InvalidAdmin(admin);

    emit AdminChanged(StorageSlot.getAddressSlot(ERC1967Utils.ADMIN_SLOT).value, admin);
