...
const engine = new projects.Service("compute", { service: "compute.googleapis.com" });
...
const network = new compute.Network(
  "network",
  { name: `${stack}-network`, autoCreateSubnetworks: false },
  { dependsOn: engine },
);
const subnet = new compute.Subnetwork("network", {
  region: location,
  network: network.id,
  name: `${stack}-network`,
  ipCidrRange: "10.0.0.0/20",
});
const address = new compute.Address(
  "egress",
  { name: `${stack}-egress`, region: location },
  { dependsOn: engine, protect: true },
);
...
export const egressIp = address.address;
...
export const services = Object.fromEntries(
  Object.entries(modules.services).map(([name, fields]) => {
    ...
    const target = new cloudrunv2.Service(
      ...
      { dependsOn: [run, gateway, ...service.dependencies] },
    );
    ...
    `${name}-check`,
    {
      ...
      template: {
        template: {
          ...
          maxRetries: 0,
          timeout: config.get(`${name}Timeout`) ?? "60s",
        },
      },
    },
    { dependsOn: [run, gateway, ...dependencies] },
  );
  return {
    name,
